...
| **T15** | The `mv_webpane` cookie (minted by `POST /api/webpane-token`, accepted by the two `/api/webpane/*` content routes in place of the bearer header) reopens the CSRF surface T4's "header, not a cookie" mitigation was written to prevent | Three independent bounds keep this narrow, all asserted directly against the real `Set-Cookie` header by test: (a) `SameSite=Strict` — never sent on any cross-site request, so a malicious site cannot trigger an authenticated request to these routes at all; (b) `Path=/api/webpane/` — never sent to, and grants no access to, any other route, including `/api/webpane-token` itself (verified: a valid cookie presented to the mint endpoint or to `/api/sessions` still 401s); (c) the cookie's capability is a `WebpaneTokenStore` entry bound to exactly one resource (one port, or one path) for 5 minutes — a cookie minted for one resource is rejected against any other (cross-kind and cross-value, verified against the real token store, not a stub). Blast radius: strictly narrower than T4's original concern, which was the main bearer token being CSRF-reachable for the full API surface. (microviber-track-b-2, 2026-08-28) |
