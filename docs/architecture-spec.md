...
| F19 | **Web Push works end to end on a real device (Android / Chrome); iOS still unverified** (story push-notification-dispatch-1 — laptop-side spike 2026-09-06, on-device test 2026-09-07) | Transport: `tailscale serve` fronts the daemon at `https://<name>.ts.net` with a real Let's Encrypt cert (`tailscale cert`, INSTALL.md Stage 2) — not self-signed, so service-worker registration and `PushManager.subscribe` are available to the installed PWA. Outbound: from the laptop, `POST` probes to the three browser push services all answered (FCM 401, Apple `web.push.apple.com` 403, Mozilla 404 — reachable; 4xx is the expected answer to an unauthenticated probe). `web-push` 3.6.7 offline: `generateRequestDetails` produced `Content-Encoding: aes128gcm`, `Authorization: vapid t=…, k=…`, `TTL`/`Urgency`/`Topic` headers, and a 194-byte ciphertext with no plaintext in it. Online: `sendNotification` to a synthetic FCM subscription left the machine and came back `WebPushError 410 "push subscription has unsubscribed or expired"` — so the sender treats 404/410 as "gone → prune". Consequence recorded as **T19**: this is the daemon's first and only outbound network call, opt-in via `MV_VAPID_*`. **Real device receiving a push: PASS (2026-09-07, Android / Chrome).** An installed PWA subscribed through the in-app opt-in, was backgrounded, and received a notification for an idle laptop session; tapping it opened that session. One defect was found and fixed by that test: `showNotification` set neither `icon` nor `badge`, so Android Chrome rendered the push under its own logo — both now point at the manifest's `/icon-192.png`, and `pwa/test/sw.test.ts` pins them along with the rest of the daemon↔`sw.js` payload contract. Not yet exercised on iOS, so T19's silent-dismiss question remains open there. |
...
| `services/` | Cross-cutting service wiring (audit log, push notify loop), composed for `api/`. `notify-dispatch.ts` — the 5s daemon-side poll that feeds `listSessions()` into `NotifyPolicy.reconcile()` and fans every returned intent out to every stored push subscription (a `gone` outcome prunes that one subscription). Its first cycle only **primes** the policy — intents are discarded — so a restart never re-notifies already-idle sessions; `index.ts` runs that priming cycle at t=0 (`loop.tick()`) because `startNotifyLoop` itself only schedules the `setInterval`, which would otherwise leave the priming a full poll interval late. A cycle already in flight is never stacked on, and a cycle with ZERO stored subscriptions returns before `listSessions()` — that call is a synchronous full discovery plus per-session transcript scan, and this loop runs for as long as `MV_VAPID_*` are set, so nobody subscribed must cost nothing; skipping also un-primes the loop, so the first cycle after a subscription arrives primes again and only the next one may dispatch (a phone that just opted in is never buzzed about already-idle sessions). (push-notification-dispatch-1) |
...
| `lib/push-sender.ts`, `lib/push-subscription-store.ts` | Web Push (push-notification-dispatch-1). `push-sender.ts` wraps `web-push`: VAPID-signed, aes128gcm-encrypted `sendNotify`/`sendDismiss` with an RFC 8030 `Topic` per session tag (a dismiss replaces an undelivered notify at the push service), TTL 1h, a 10s `timeout` — `web-push` arms its socket-inactivity handler only when that option is set, and `https.request` has no default, so without it a push service that accepts the POST and never answers would leave the send pending forever and stall the notify loop — and outcomes `ok \| gone \| failed` (404/410 ⇒ gone). `push-subscription-store.ts` persists browser subscriptions to `~/.microviber/push-subscriptions.json` (0600, atomic write, zod-validated, fail-closed like `devports.json`, keyed by endpoint, at most 5 with the oldest evicted); `list()` hands out a snapshot, since the notify loop iterates it across awaits while a concurrent subscribe can mutate the store. Adjacent to the adapter layer but NOT inside `lib/claude-adapter/` — it knows nothing about Claude Code. |
...
- `ownership.ts` — `OwnershipRegistry`: bookkeeping for which session ids are currently
  ...
  racing takeover calls for one session can never double-spawn, and an identity-aware
  `reap` so a superseded child's late exit never drops the survivor's entry (T17,
  takeover-race-hardening-1).
- `notify-policy.ts` — idle-notification policy (§5 of the functional spec). Wired into a real sender by
  `services/notify-dispatch.ts` (push-notification-dispatch-1); the policy itself is unchanged.
...
**Web Push, client side (push-notification-dispatch-1):** `lib/push.ts` (fetch
`/api/push/config`, `pushManager.subscribe` + idempotent re-sync, VAPID-key-rotation
check, the `/?session=<id>` and `open-session` deep-link helpers, and clear-on-open),
`hooks/usePushOptIn.ts` (whether and when to *offer* — paired, supported, undecided,
daemon has keys, not previously dismissed; an already-granted permission means a silent
re-sync, never a prompt on cold load), `components/PushOptIn.tsx` (the one-line offer),
all wired in `App.tsx`. `pwa/public/sw.js`'s three handlers — `push`, `message` and
`notificationclick` (show a tagged notification, close a tag on a `dismiss` payload or
client message, focus-or-open the PWA on a tap) — pre-dated that story, but **the file
was modified by it**: `showNotification` gained `icon`/`badge` (a real-device finding —
without them Android Chrome renders the push under its own logo), and `sw.js` got its
first test coverage at all, `pwa/test/sw.test.ts`, which pins the daemon↔service-worker
payload contract from the client side. Read that diff rather than skipping it: `sw.js` is
the last hop of the push path and the one file where a real-device defect has actually
been found.
...
| `/api/sessions/:id/takeover` | POST | bearer | Resume an idle laptop-started session as a daemon-owned process (`claude --resume <id>`), making it writable. `FORBIDDEN` (403) if the session is not idle or is on an unrecognized Claude Code build; `NOT_FOUND` (404) for an unknown session id. A request that joins an already in-flight takeover of the same session receives that takeover's outcome instead of being idle-gated itself (T17). |
...
| `/api/push/config` | GET | bearer | `{ enabled, publicKey }` — whether push is configured (`MV_VAPID_*` set and a subscription store present) and the VAPID public key the PWA passes to `pushManager.subscribe`; `publicKey` is `null` when disabled. Fetched at runtime, not baked into the PWA build (keys are per-install). (push-notification-dispatch-1) |
| `/api/push/subscribe` | POST | bearer | Body is exactly `PushSubscription.toJSON()` (`{ endpoint, expirationTime?, keys: { p256dh, auth } }`, strict, endpoint ≤ 2048 chars). `endpoint` must be a public `https:` URL: a URL carrying credentials, `localhost`/`*.localhost`, an IPv4 or IPv6 literal, `*.local`, `*.ts.net`, `*.internal`, `*.home.arpa`, a single-label host, or any port other than `443` is rejected 400 `INVALID_INPUT` — a root-anchored trailing dot is stripped before the hostname checks, so `https://localhost./` cannot slip past them, and the port pin is what stops a public loopback-resolving name like `https://mv.localtest.me:8730/` (T19). Upserts by endpoint into the on-disk store; 400 `INVALID_INPUT` "push notifications are not configured on this daemon" when `MV_VAPID_*` are unset. Returns `{ ok: true }`. (push-notification-dispatch-1) |
...
## 5. Transport & security (threat model T1–T19)
...
`0.0.0.0`. It is not started by the build or the install runbook and does not run at boot
**by default**: it is started deliberately and stopped when not needed, so the exposure
window is minutes-to-hours of chosen use. A user may opt in to auto-start
(`./bin/microviberd autostart on`, T18), which widens that window to the whole logged-in
session — an informed trade the command prints every time it is made, reversed by one
command.
...
| **T6** | Lost or stolen phone with a paired PWA | Device lock screen is the first barrier. Revocation is immediate: rotate the daemon token and restart — every paired client dies at once. **What a found phone can show grew (microviber-track-c-1, 2026-09-08):** the rendered transcript now includes full tool inputs, tool results and thinking text, so it may hold file contents and command lines it previously did not — and token rotation cannot un-render what the device already displayed. See T9. |
...
| **T8** | Token leaks via logs, URLs, or screenshots | Token travels in a header, never a query param or body. The pairing URL carries it in the fragment, which browsers never send to a server. **Screenshot scope grew (microviber-track-c-1, 2026-09-08):** a transcript screenshot can now contain a tool result or an `Edit` payload, i.e. file contents, not just conversation. See T9. |
| **T9** | `~/.claude` secrets exfiltrated through the API | The daemon reads `peerToken` values for discovery only and never returns or logs them; `SessionSummary` is an explicit allowlist of fields. This mitigation is about the API response shape and is unaffected by microviber-track-b-4 (T11/T16) below — that story lets transcript content choose a local-file *path* to display in the PWA, not a field the API returns; the two are different surfaces, but see T16 for why a maliciously crafted transcript link could still surface unrelated on-disk secrets to the screen of whoever taps it. **Widened (microviber-track-c-1, 2026-09-08) — this row's "API response shape" framing no longer covers the whole surface.** Before this story the only tool detail that left the laptop was `summarizeToolInput`'s single 120-character summary string. The transcript response now carries the **complete `input` object of every tool call** (a `Bash` command line, an `Edit`'s `old_string`/`new_string`, a `Write`'s whole `content`, a `MultiEdit`'s `edits`, a `TodoWrite`'s todos), **every tool result's text** including stderr, and **the model's thinking text** — none of which the phone previously saw. A `Read` of `.env`, `~/.ssh/id_rsa` or `~/.claude/.credentials.json` therefore now puts up to `TOOL_PAYLOAD_MAX_CHARS` of that file on the wire and on the phone screen. `SessionSummary` is still an explicit allowlist and `peerToken` is still never serialized — that half of the mitigation is intact and was re-verified — but the transcript payload is not an allowlist and was never intended as one. The delta lands on **T6** (a lost phone's rendered transcript may hold credential-bearing content, and rotating the token cannot un-render what was already displayed) and **T8** (a screenshot may now contain a credentials file). Containment, verified rather than assumed: nothing is persisted client-side (`localStorage` holds only the pairing token, push opt-in and webpane targets — no code writes events), the daemon runs `Fastify({ logger: false })` and logs no event content, and the payload travels only over the bearer-gated tailnet tunnel. So the exposure is in transit plus the phone's browser memory. **Accepted, with the tension named:** `services/audit-log.ts` sha256-hashes prompt text specifically so secrets never reach disk, and this feature's own design spec rejected PTY capture because "capturing a PTY stream would write secrets to disk" — the read path now streams the same class of content verbatim to the phone. That is the point of a mirror of a coding agent, and a single-user personal tool whose bearer already grants arbitrary code execution on the laptop (§5 preamble) gains no new capability from it; recorded here so no future reader mistakes this row's allowlist language for a bound on what the transcript reveals. |
...
| **T11** | Prompt injection via transcript content | MicroViber never executes, auto-sends, or acts on transcript content; it only displays it. **Narrowed (microviber-track-b-4, 2026-08-31):** "only displays it" now needs a caveat — transcript-rendered markdown links are classified and a local-file/dev-server link is one tap away from the Web pane's `POST /api/webpane-token` + `GET /api/webpane/localfile` (see T16). The daemon is still never told to *execute* transcript content; the PWA now lets transcript content pick which already-accepted-as-readable path gets *displayed*. See T16 for the containment this relies on. **Narrowed (askuserquestion-answer-mechanism, 2026-09-04):** the daemon now echoes model-authored `AskUserQuestion` option labels back into the session as a user prompt — only on an explicit user tap, only labels validated exactly against the pending question's own options (`lib/claude-adapter/ask-user-question.ts`'s `validateAnswer`), in a fixed format, length-capped; the composer already allows arbitrary user text on the same route, so no new capability is granted. **The "fixed format" claim is enforced, not just asserted (code review round 1, 2026-09-04):** `AskUserQuestionInputSchema` rejects control characters (including newlines) and duplicate option labels at detection time, so a poisoned question can't smuggle extra lines into a composed answer via a multi-line label, and an answer can't be ambiguous between two identically-labelled options. **Narrowed further (askuserquestion-answer-mechanism-3, 2026-09-08):** that story added a parser over the *answer stub* Claude Code writes into the transcript, i.e. a second reader of model-authored transcript text. Two structural properties keep it inside "only displays it", both traced consumer-by-consumer during the story's security review rather than asserted: (a) **the output alphabet is closed** — a parsed label is always an element of the pending question's own `options[].label`, never a slice of the stub, so it grants no text the composer's own route doesn't already grant; and (b) **its output reaches only the card's highlight and caption** — `selectedLabels` is never seeded into the PWA's pick state, a resolved card is non-interactive, and the write path re-derives the pending question from the live transcript and validates *client-supplied* selections, so no parsed label reaches a prompt, an audit entry, or storage. The parse is also bounded by a step budget (see §6) because an unbounded one was measured stalling the single-threaded daemon for minutes on schema-legal input. |
...
| **T17** | Two takeover requests for the same session race — a network retry, a double-tap, or two paired devices both `POST /api/sessions/:id/takeover` before either finishes. Each sees no registry entry yet (the first `spawn()` is still awaiting), each passes the idle gate, each spawns its own `claude --resume` child; `acquire` keeps only the last handle, so the first child is orphaned — never killed (`release` only kills whatever handle the registry currently holds) and still holding a live stdin into the session; worse, both children's `onExit` callbacks are wired to `reap(sessionId)`, so when the orphan eventually exits it deletes the *survivor's* registry entry and silently flips a live owned session back to read-only. Both callers hold the bearer, so this is not an attacker's move — but it is a network-reachable path to a process the daemon has forgotten it owns, which none of T1–T16 cover. (Today the adapter resolves the spawn synchronously — `startTakeoverSession` uses `_resolveImmediately` — so the pre-fix check→acquire window was microtasks wide and a second HTTP request could not land inside it; the lock makes the guarantee independent of the adapter's resolve timing rather than an accident of it.) | `OwnershipRegistry.coalesceTakeover` (`domain/ownership.ts`): a per-session in-flight `Map<sessionId, Promise<OwnedSessionHandle>>`. `takeover()`'s whole sequence — the `existing?.alive` short-circuit, the idle gate, `spawn()`, `acquire()` — runs inside it, so a second caller for a session already mid-takeover awaits the SAME promise and re-runs none of it. The entry is dropped when the call settles (success or failure), so a failed spawn can't wedge later attempts behind a rejected promise; the lock is keyed per session, so takeovers of different sessions never wait on each other. Sequential idempotency (an alive owned handle is returned without re-spawning) is unchanged. Covered by `daemon/test/ownership.test.ts`: two concurrent same-session calls → one spawn, both get the same handle; a racing caller whose own state snapshot would fail the gate still gets the in-flight handle; a rejected spawn rejects both callers once and unblocks the next attempt; different sessions spawn independently. A caller that joins an in-flight takeover receives its outcome even if its own state snapshot has since left idle — consistent with the existing rule that once a session is owned, state no longer gates (the takeover it joined was initiated by a caller that passed the gate). Complement (same story, from the final whole-branch review): the lock closes the concurrent-spawn path; the other way to end up with two children for one session — handback, then a re-takeover that acquires a fresh handle before the old process has actually exited — is closed by an identity-aware `reap`: `acquire` binds the handle into its `onExit`, and `reap(sessionId, handle)` ignores a late exit whose handle is no longer the registry's current entry, so the survivor is never dropped (covered by the "identity-aware" tests in `daemon/test/ownership.test.ts`). Residual (not closed here): a handback that lands while a takeover of the same session is in flight is a silent no-op — `release` finds no entry yet and the takeover then acquires — tracked as [#45](https://github.com/yarivsnapir/MicroViber/issues/45). (takeover-race-hardening-1, 2026-09-06 — found during story/microviber-2's security review as informational/pre-existing, filed as GitHub issue #4) |
| **T18** | **Auto-start (opt-in) makes the daemon an always-on-while-logged-in service.** The window is no longer chosen use: anyone holding both factors — tailnet membership and the bearer token — can take over idle sessions whenever the laptop is logged in, with no human having just started the daemon; the daemon also comes back unattended after a crash; and its startup output, which includes the pairing URL (T8), now lands in a persistent log. | Default stays **off**: the service is installed only by an explicit `microviberd autostart on` — never by build, install, or `start` — and that command prints this trade-off on every run. Auto-start changes *when* the daemon runs, not *who* can reach it: same bind whitelist, same Host/Origin/bearer gates, same token-rotation kill switch (T6). `status` and `autostart status` always show the auto-start state; `stop` closes the window at once until next login; `autostart off` removes the service. The service runs as the user, never as root or system-wide. The runner sets the log directory to `700` and the log to `600` on every `autostart on`, before the manager first opens it; on Linux the journal is user-scoped. (opt-in-autostart, 2026-09-06) |
| **T19** | **The daemon's first outbound network call.** Web Push requires the daemon to `POST` to a third-party push service chosen by the phone's browser (Google FCM, Apple, Mozilla, Microsoft) — outside the tailnet, on the public internet. Two exposures: (a) what leaves the tailnet, and (b) a bearer holder pointing that outbound call at an internal target by registering a crafted `endpoint` (SSRF into loopback/tailnet/LAN — e.g. `https://127.0.0.1:8730/…` or a `.ts.net` peer). | (a) **Payload is end-to-end encrypted** (RFC 8291 `aes128gcm`, keys held by the phone's browser) — the push service sees ciphertext, the endpoint, timing, size, and the two headers this daemon sets on every send (`push-sender.ts`) — `Topic`, a stable sha256 pseudonym **per session**, which lets the service count distinct sessions and correlate each one's cadence, and `Urgency`, `high` for a notify and `normal` for a dismiss, which distinguishes an idle alert from a cancellation in cleartext — but never the session title or status line; the VAPID private key never leaves the laptop. **Opt-in**: nothing is sent, and no sender is even constructed, unless `MV_VAPID_PUBLIC_KEY`/`MV_VAPID_PRIVATE_KEY` are set (`index.ts`); without them the daemon's network posture is exactly what it was before this story, and it says so at startup (`Push notifications: disabled … No outbound calls are made while disabled.`). The subscription file (`~/.microviber/push-subscriptions.json`) is 0600 next to the bearer token — with the VAPID private key it is enough to push to that phone. (b) `isSafePushEndpoint` (`schemas/api.ts`) is enforced at the API boundary AND re-validated when the store file is loaded (the stored-subscription schema extends the request schema, so the same endpoint refinement runs on load): `https:` only, no credentials in the URL, and the hostname — after a root-anchored trailing dot is stripped — must not be `localhost`/`*.localhost`, an IPv4/IPv6 literal, `*.local`, `*.ts.net`, `*.internal`, `*.home.arpa`, or a single-label name; and the port must be absent or `443` (`new URL()` normalizes an explicit `:443` away, and all four real push services are 443-only). **The hostname rules are syntactic, and that is their honest limit:** a *public* multi-label DNS name that resolves into private address space — `*.nip.io`, `*.sslip.io`, `localtest.me` — still passes them, so they do not by themselves deliver "no loopback / IP literal / tailnet / LAN". What contains that is the pinned port, which blocks the reachable form of the trick (aiming the daemon at `https://mv.localtest.me:8730/…`, i.e. at itself, or at any other LAN port — `web-push` forwards the endpoint's port verbatim), plus the mandatory TLS validation on the outbound call (the target must terminate valid TLS for the attacker-chosen hostname; `web-push` never disables verification) and the bearer requirement to register an endpoint at all. Resolving the hostname and checking the ADDRESS was considered and deliberately not done: it costs a DNS round trip on every subscribe and still leaves an inherent TOCTOU window, since the name is resolved again when the request is actually made. A successful subscribe is now recorded through the audit sink — endpoint **host only**, never the path, which is bearer-secret-like — so the configured outbound target is at least visible after the fact. Residual, accepted — and *"public"* overstates the containment: the hostname rules are syntactic and the port pin only excludes non-443, so a bearer holder who controls any DNS name can point that name at a loopback/tailnet/LAN address and still obtain a publicly-trusted certificate for it through a DNS-01 challenge, which needs no inbound reachability at all. The honest statement of the residual is therefore: a bearer holder can make the daemon POST small encrypted blobs to **any https host reachable from the laptop on port 443 — public *or internal* — that terminates TLS valid for an attacker-chosen name**, and read back a coarse ok/gone/failed status oracle. The security delta is nonetheless nil: the same bearer already injects prompts into a `--dangerously-skip-permissions` session (§5 preamble), i.e. it is already arbitrary code execution on the laptop, so this grants no new capability against the laptop or the tailnet. **Known platform gap (AC4) — every platform, not just iOS:** the PWA subscribes with `userVisibleOnly: true` (`pwa/src/lib/push.ts`) and `sw.js`'s dismiss branch returns *without* calling `showNotification`, so every **delivered** dismiss is a silent push in breach of that contract — including on the Android/Chrome build this branch actually verified. Chrome enforces it with a per-origin push budget and, once that budget is spent, substitutes its own generic "site updated in the background" notification; iOS Safari may instead throttle or revoke the subscription outright. Mitigations already in place: every notify carries a 1h TTL, and the PWA clears a session's notification the moment it is opened. **The per-session `Topic` is not a mitigation for the case that actually happens:** it only replaces a notify still *queued* at the push service, whereas the common sequence is notify delivered and shown, user walks back to the laptop, and the dismiss then *must* be delivered to close it. **Verified on a real device (Android / Chrome, 2026-09-07): a push is delivered and its tap opens the right session. The iOS half is still unverified** — no iOS device has exercised this, so whether Safari throttles or revokes on the silent dismiss push is still open. (push-notification-dispatch-1, 2026-09-06; on-device outcome and the platform-gap wording updated 2026-09-07) |
...
- **Bound untrusted-input readers by work, not only by size.** A parser over
  content MicroViber did not author — transcript text, on-disk files, anything a
  model or another process wrote — must cap the *work it performs*, not just the
  *bytes it looks at*. A length bound only fails closed on inputs whose length
  someone predicted; a step budget fails closed on inputs nobody predicted. Where
  a size cap and a work budget interact, say so where the cap is defined: neither
  is individually sufficient, and a cap described as "cost-only" stops being
  cost-only once a budget can exhaust inside it. Establish the budget's headroom
  by measuring the largest *legitimate* input the schema admits and the ceiling
  cost at the budget, and record both measurements next to the constant — this
  standard exists because `AskUserQuestion`'s answer-stub parser shipped a
  length-bounded-but-unbudgeted scan whose cost was super-linear inside the
  bound, measured at minutes of synchronous event-loop time on schema-legal
  input while its own documentation recorded milliseconds
  (askuserquestion-answer-mechanism-3, 2026-09-08). Exhaustion must degrade to
  the reader's existing "can't tell" result, never to a partial or guessed one.
...
- **Adapter quarantine.** Nothing outside `daemon/src/lib/claude-adapter/` touches Claude
  ...
  *bookkeeping*, restated; the actual decisions — is this an `AskUserQuestion`, does this
  ...
  there. Current instances:
  `docs/features/askuserquestion-answer-mechanism/stories/story-3-manual-test.ts`, and
  `docs/features/microviber-track-c/stories/story-1-manual-test.ts` (microviber-track-c-1,
  2026-09-08) — the second is strictly narrower than the carve-out and needs **no** eslint
  `ignores` entry: it reads nothing from `~/.claude` and walks no filesystem at all, it
  hand-authors a synthetic transcript in memory and drives the adapter's own exported
  `parseChunk` over it. It is listed anyway because it does author transcript-entry
  vocabulary from outside the adapter, which is the drift this paragraph exists to catch;
  `daemon/test/tail.test.ts` has always done the same, so adapter *tests* are the
  established reading of that line rather than an exception to it.
