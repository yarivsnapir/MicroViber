...
| `/api/push/subscribe` | POST | bearer | Body is exactly `PushSubscription.toJSON()` (`{ endpoint, expirationTime?, keys: { p256dh, auth } }`, strict, endpoint ≤ 2048 chars). `endpoint` must be a public `https:` URL: a URL carrying credentials, `localhost`/`*.localhost`, an IPv4 or IPv6 literal, `*.local`, `*.ts.net`, `*.internal`, `*.home.arpa`, or a single-label host is rejected 400 `INVALID_INPUT` — and a root-anchored trailing dot is stripped before those checks, so `https://localhost./` cannot slip past them (T18). Upserts by endpoint into the on-disk store; 400 `INVALID_INPUT` "push notifications are not configured on this daemon" when `MV_VAPID_*` are unset. Returns `{ ok: true }`. (push-notification-dispatch-1) |
