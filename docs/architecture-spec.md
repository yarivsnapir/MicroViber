...
all wired in `App.tsx`. **`pwa/public/sw.js` was not modified by that story** — its
`push`, `message` and `notificationclick` handlers (show a tagged notification, close a
tag on a `dismiss` payload or client message, focus-or-open the PWA on a tap) already
existed, so there is no service-worker diff to go looking for.
