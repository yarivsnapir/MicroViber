...
story-8  (merged: covers former story-9 + story-10 scope in one story/PR)
...
- **Story 8 carries real schedule risk**: it opens with a mandatory empirical spike (verifying `tool_result`-over-stdin actually works) whose outcome isn't known yet. If it fails, criterion 15 (answer-submission) is dropped, not the whole story — see story 8's Technical Notes for the fallback.
- **2026-09-02: stories 8, 9, and 10 were merged into one.** GitHub issue #15 was edited to combine the spike, detection, state-fix, and PWA-UI work (originally split across three stories) into a single story/PR, and issues #16/#17 were closed. `story-9.md` and `story-10.md` are kept for historical reference only, marked `status: superseded`; `story-8.md` now carries the full combined scope and is the one to implement.
