...
| 8 | AskUserQuestion support: detection, takeover fix, and PWA answer flow | microviber | L | — | todo | [#15](https://github.com/yarivsnapir/MicroViber/issues/15) |
...
story-5   (independent)
story-7   (independent)
story-8   (independent — merged from the former stories 8/9/10)
...
- **Rollout safety** (per spec-to-stories rule 1b): every prefix of this sequence is safe to ship in isolation. Stories 1-2 are purely additive backend work (new fields/routes nothing yet calls); story 3 is the first to actually consume them; story 4 depends on 3's `navigateWebPane`; story 6 depends on 3's `CaretButton`. Stories 5 and 7 are fully independent UI polish.
- **Story 8 merges the former stories 8, 9, and 10** (2026-09-02) into one story spanning detection → the actual takeover bug fix → the PWA UI, since none of the three had an independently shippable intermediate state that wasn't either "detects a thing nothing reads yet" or "reads a thing that doesn't exist yet." Do the work in the order its acceptance criteria are numbered. It still opens with a mandatory empirical spike (verifying `tool_result`-over-stdin actually works) whose outcome isn't known yet — if it fails, only the answer-submission criterion is dropped, not the whole story; see story 8's Technical Notes for the fallback. GitHub issues #16 and #17 (the former stories 9 and 10) were closed as merged into #15.
