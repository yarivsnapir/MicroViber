...
**AC15 resolution note (2026-09-03), F16/F17 (architecture-spec.md §2):** criterion 1's spike recorded PASS at the *transport* level — a `tool_result` frame written via `--resume`'d stdin does land, correctly attributed. A Task 7 was added mid-implementation to actually use that mechanism (the originally-planned plain-text `send()` path for AC15 never produces a matching `tool_result` at all — Claude Code does not backfill one, confirmed empirically). But live testing then found a deeper issue (F17): `claude -p --resume` unconditionally injects its own `"Continue from where you left off."` handshake before processing any stdin content, so even a correctly-landed `tool_result` doesn't produce a coherent model continuation — the daemon-side bookkeeping (`pendingQuestion` clearing, AC2-10) is genuinely correct and verified, but AC15's "the session continues normally" promise does not hold for any mechanism tried so far. Per AC15's own FAIL-branch instruction ("do not invent an alternative submission mechanism inline without returning to brainstorming first"), the PWA ships with options rendered inert (read-only) even when taken over — AC12-14 are fully met, AC15 is **not yet met** and needs a follow-up brainstorming session on the submission mechanism specifically. The daemon-side write plumbing (`sendAnswer`/`submitAnswer`/`toolResultFrame`) stays in the codebase, unused by the UI, for whenever a working mechanism is found. **Follow-up shipped as `docs/features/askuserquestion-answer-mechanism/` (F18).**

**Story closed (2026-09-06):** the follow-up shipped in full — `askuserquestion-answer-mechanism-1` (daemon, PR #33) and `askuserquestion-answer-mechanism-2` (PWA, PR #34) — so AC15 is now met via that feature rather than inline here. This story's own status and GitHub issue #15 were stale (still open/in-progress) after the follow-up merged; reconciled now. A residual known limitation from story-2's own code review (cross-question option-label highlighting when two questions share a label) is tracked separately as `askuserquestion-answer-mechanism-3`.

## Manual Test Checklist
- [x] Complete the spike (criterion 1) and record its outcome before writing any other code in this story. — F16, PASS (transport).
- [x] Run `npm run typecheck && npm run lint && npm test` from `microviber/` — all green. — 313 daemon + 122 pwa tests passing.
- [x] Verification script: feed `scanTranscriptMeta` a real transcript excerpt containing an `AskUserQuestion` tool_use with no matching tool_result, confirm `pendingQuestion` is populated with the correct question text/options; feed it the same transcript plus a matching tool_result entry, confirm `pendingQuestion` is `null`. — run directly, matches exactly.
- [x] **Before implementing the state fix**, reproduce the bug on current `main`: get a real session to call `AskUserQuestion`, confirm the daemon reports it as `working` and that `POST /api/sessions/:id/takeover` 403s. — covered by `deriveState`'s regression-guard unit tests (pre-fix behavior contrast); not additionally reproduced against a live HTTP endpoint.
- [x] **After implementing**, repeat: confirm the session now reports `awaiting-input` and takeover succeeds. — verified against real transcript data end-to-end (ran the actual `tail.ts`/`transcript-meta.ts` code against a real session's transcript: `resolved:true`, `pendingQuestion:null`) and confirmed live in the PWA (session list shows `awaiting-input` dot, takeover succeeds).
- [x] Confirm a session with a genuinely open non-question tool call (e.g. mid-`Bash`) still correctly reports `working`, not `awaiting-input` — no regression to the existing open-turn heuristic. — covered by automated regression tests.
- [x] On the phone, confirm the session list shows the distinct `awaiting-input` dot and the transcript shows the question expanded with real options. — confirmed live (real device/browser).
- [ ] ~~If the spike passed: tap "Take over," then tap an option; confirm it submits and the session continues normally.~~ **Superseded by AC15's resolution note above (F17):** confirmed live, multiple real sessions — tapping does correctly write the answer and clear `pendingQuestion`, but the model does not coherently continue (Claude Code's own `--resume` startup handshake intervenes first). Per AC15's FAIL-branch instruction, options ship inert instead; this checklist item cannot pass as originally written until a follow-up brainstorming session finds a working submission mechanism.
- [x] Confirm a resolved question elsewhere in the transcript renders read-only with the actually-chosen option highlighted, not tappable. — confirmed live and by automated tests.
- [x] Confirm a non-`AskUserQuestion` tool call is unaffected — still collapses to one line as before. — covered by automated regression tests in both `tail.ts` and `transcript-meta.ts`.
