...
**Real-device test round 1 (2026-09-07) — findings:**
- **The notification had no app icon (fixed).** `sw.js`'s `showNotification` set neither `icon` nor `badge`, so Android Chrome rendered the push under its own logo and a MicroViber notification was unattributable on the lock screen. Both now point at the manifest's existing `/icon-192.png`. This also brought `sw.js` its first test file (`pwa/test/sw.test.ts`), closing the whole-branch review's finding that the daemon↔sw.js payload key names were pinned only against themselves.
- **The notification in the report was not this feature's.** "Tap to copy the URL for this app", with Share / "Open in browser" actions under a Chrome icon, is Chrome's own notification for a site running in standalone mode without a real install. Confirmed by three independent facts: the daemon serving the phone was built from another branch and its `dist/` contained no push code, no `~/.microviber/push-subscriptions.json` existed, and its log had never printed a `Push notifications:` line. The push path had not run on the device at all. Chrome's notification is not suppressible from app code — a proper WebAPK install (Chrome menu → "Install app") avoids it, an "Add to Home screen" shortcut does not.
- **Testing this before merge needs a daemon built from this branch.** `stories/story-1-phone-test.sh` builds the branch, borrows port 8730 from the launchd agent, and restores the agent on exit. It never reads or edits `.env` or the bearer token.
- Still unverified on a device: that a push actually arrives, that tapping it opens the right session, and whether iOS throttles the silent dismiss push (T19's recorded gap).
...
- **T19** added: first outbound call, opt-in, E2E-encrypted, endpoint SSRF guard (`isSafePushEndpoint`, which also strips a root-anchored trailing dot so `https://localhost./` can't bypass the hostname rules).
