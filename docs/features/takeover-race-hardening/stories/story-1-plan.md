...
- ~~The stale "threat model T1–T12" wording in the §5 heading and in `CLAUDE.md`~~ — flagged by the code review (2026-09-06) and corrected to T1–T17 in the review-fixes commit after all.
- The story's live checks (two near-simultaneous real `curl` takeovers; handback → immediate re-takeover) were automated as `story-1-manual-test.sh` (committed; run by the user 2026-09-06, 12/12 ✅). Step 4 of that script is a non-regression check — it also passes on pre-fix code — step 5 (AC7) is the discriminating one.
