...
One sharp edge inside that parse, worth stating because getting it wrong costs a *wrong* highlight rather than a degrade: the value's closing `"` is **not** simply the first `"` after the anchor. `schemas.ts` types a label as `TrustedText(500)` (control characters only are rejected), so a label may contain `"`, and truncating at the first inner quote can land exactly on a **shorter label of the same question** — e.g. options `Yes` and `Yes"maybe`, answer `Yes"maybe`, truncation `Yes`, which would highlight the option the person did not pick. So every `"` at or after the anchor is tried as the close and the pair is accepted only when exactly **one** candidate parses as a run of that question's own labels; two candidates mean "can't tell" and the whole call degrades. A second candidate requires a label containing `"`, so an ordinary stub always has exactly one and is never degraded by the check.
