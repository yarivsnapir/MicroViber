...
**Empirical status of clause (a)'s multi-question case (AC2).** No fixture in this repo records a real MULTI-question answer stub — architecture-spec F16 captured only a single-question one (`content: "Yes"`), and the `'Yes, No'` in the tests is a synthesized value. The rule above is written to be correct either way: it parses the stub exactly or says "can't tell", and every "can't tell" degrades to an unhighlighted card, never to a wrong highlight. To settle it against real data, run the probe at `stories/story-3-manual-test.ts`, which drives the **shipped** `detectAskUserQuestion` + `isResolvingUserEntry` over `~/.claude/projects` (override with `MV_PROBE_ROOT`) and redacts labels unless `--raw` is passed. It reports CONFIRMED / PARTIAL / INCONCLUSIVE; record a PARTIAL result's stub shape here. **As of story `askuserquestion-answer-mechanism-3` the probe has been exercised only against hand-authored fixtures** (both verdict branches), not against a real transcript corpus — so the multi-question stub's real shape remains unobserved, and clause (a)'s conservative "can't tell" is what ships.
