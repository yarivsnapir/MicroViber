...
| 50 × 500-char labels sharing a 490-char prefix, select-all value | 24.8 KB | **9 288 ms** | 34 ms |
| a **genuine** select-all stub — nothing crafted but model-authored dense labels, each selected once, `", "`-joined exactly as the CLI writes it | 25.2 KB | **999 ms**, attributed | 10 ms, `undefined` |
...
So the parse now also carries a **total budget of label-matching steps for one entry** (`TAKE_LABEL_STEP_BUDGET`, 8 000), spanning every question, every candidate close and both walks; exhausting it yields `undefined` for the whole call. Unlike a second length bound this is **result-changing by design** — the genuine row above degrades from an attribution to an unhighlighted card — and that is the trade taken deliberately: a budget fails closed on inputs nobody predicted, whereas a length bound only fails closed on inputs whose length someone predicted. Sizing, all measured: the largest answer the schema admits (4 questions × 50 options × 500-char labels, every option selected — a 100 547-byte stub) parses in **200 steps** and is pinned as still parsing; the most any one parse in the 529-test daemon suite spends is **50**; a one-pick-per-question answer whose labels are as dense as `TrustedText(500)` allows spends **2 460**. 8 000 is 40× the first of those, and the ceiling costs **34 ms** on the most expensive step shape available. What it cuts off is stated rather than left to be discovered: on that maximal select-all, one `".` pair per label still parses (5 300 steps) and two do not (10 400). Residual: this bounds one parse of one entry, so a transcript carrying *N* such occurrences still costs *N* × the ceiling per scan.
