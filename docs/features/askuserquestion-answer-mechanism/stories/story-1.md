...
3. A pending question is resolved by the first later user entry that either (a) carries a `tool_result` with its `tool_use_id`, or (b) is a human turn: has text (string content or a `text` block), `isMeta !== true`, and no `origin` field. The `isMeta: true` handshake turn and `origin.kind: 'task-notification'` entries never resolve a question. The interruption marker does.
