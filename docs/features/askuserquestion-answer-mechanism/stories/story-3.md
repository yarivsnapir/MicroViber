...
7. **(Added 2026-09-08, after AC2's empirical check inverted its premise.)** `labelsFromToolResult` recognises the laptop's REAL answer-stub format — `… "<question text>"="<selected label>", "<question text>"="<selected label>" …` — by anchoring on each pending question's own `question` text and matching the paired value against that question's own option labels, returning one array per question. The previously-assumed `", "`-joined bare-label run is KEPT as a fallback. Evidence (from `story-3-manual-test.ts` over 1306 real transcripts): 349 stubs did not match the bare-label assumption, in 107 distinct shapes, whose three most common (215 of 349) are exactly this pair format; the bare-label run matched only **9 of 308** single-question stubs and **0 of 50** multi-question stubs. A stub carrying a free-text ("Other") answer for any question still yields `undefined` for the whole call, preserving AC3's all-or-nothing invariant.
